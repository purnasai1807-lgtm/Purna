...
from app.services.processing import get_preview_ready_state, is_job_stale
...
class AnalyticsJobManager:
    ...
    def submit(self, cache_entry_id: str) -> str | None:
        ...
        with self._lock:
            ...
            if future and not future.done():
                logger.info("Background job already active: cache_entry_id=%s", cache_entry_id)
                return None
    ...
    def resume_pending(self) -> int:
        with SessionLocal() as db:
            pending_entries = db.scalars(
                select(AnalysisCacheEntry).where(AnalysisCacheEntry.status.in_(("queued", "preview_ready", "processing")))
            ).all()
            ...
            for cache_entry in pending_entries:
                ...
                if cache_entry.status == "processing" and is_job_stale(cache_entry):
                    ...
                    cache_entry.progress = max(cache_entry.progress, get_preview_ready_state(cache_entry.processing_mode)[0])
                    cache_entry.progress_message = "Resuming analytics after an interrupted background job."
    ...
    def _submit_celery(self, cache_entry_id: str) -> str | None:
        with SessionLocal() as db:
            ...
            if not cache_entry:
                logger.warning("Cannot submit missing cache entry to Celery: cache_entry_id=%s", cache_entry_id)
                return None
            if cache_entry.status == "completed" and cache_entry.full_payload:
                return cache_entry.celery_task_id
            if cache_entry.celery_task_id and cache_entry.status in {"preview_ready", "processing"} and not is_job_stale(cache_entry):
                logger.info("Reusing active Celery job: cache_entry_id=%s task_id=%s", cache_entry_id, cache_entry.celery_task_id)
                return cache_entry.celery_task_id
            ...
            db.commit()
            logger.info("Celery job submitted: cache_entry_id=%s task_id=%s", cache_entry_id, task_result.id)
