...
def parse_uploaded_dataframe(filename: str, file_bytes: bytes) -> pd.DataFrame:
...
def analyze_dataframe(
    ...
    charts = generate_chart_specs(cleaned_dataframe, correlations)
...
def clean_dataframe(dataframe: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for column in dataframe.columns:
        if dataframe[column].dtype == object:
            dataframe[column] = dataframe[column].map(lambda value: value.strip() if isinstance(value, str) else value)
